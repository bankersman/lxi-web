...
import { type GspProfile, GSP_DEFAULT } from "./gsp-profile.js";
...
/**
 * GW Instek GSP-730 / GSP-9300 / GSP-9330 spectrum-analyzer driver.
 *
 * GSP SCPI closely follows SCPI-1999 with a subset of R&S FPC-style
 * commands. Key GW-Instek specifics:
 *   - `INPut:ATTenuation:AUTO` for auto-attenuation.
 *   - `SENSe:POWer:RF:GAIN:STATe` for the preamp (GSP-9300/9330 only).
 *   - `:TRACe<n>:DATA?` returns ASCII CSV.
 *
 * This driver covers the frequency / bandwidth / reference-level /
 * trace / marker surface. Channel-power, limit-lines and tracking
 * generator control are SCPI-reachable but deferred to a follow-up.
 */
export class GwInstekGsp implements ISpectrumAnalyzer {
  readonly kind = "spectrumAnalyzer" as const;
  readonly profile: GspProfile;
  ...
  constructor(
    ...
    profile: GspProfile = GSP_DEFAULT,
  ) {
  ...
  async getReferenceLevel(): Promise<SpectrumAnalyzerReferenceLevel> {
    const raw = await safeQuery(this.port, "DISPlay:WINDow:TRACe:Y:RLEVel?");
  ...
  async setReferenceLevel(dbm: number): Promise<void> {
    ...
    await this.port.write(`DISPlay:WINDow:TRACe:Y:RLEVel ${dbm}`);
  }
  ...
  async getTraceConfig(traceId: number): Promise<SpectrumAnalyzerTraceConfig> {
    ...
    const [modeRaw, detRaw, stateRaw] = await Promise.all([
      safeQuery(this.port, `TRACe${traceId}:MODE?`),
      safeQuery(this.port, `SENSe:DETector${traceId}?`),
      safeQuery(this.port, `TRACe${traceId}:STATe?`),
    ]);
    return {
      id: traceId,
  ...
  async setTraceConfig(
    ...
    if (config.mode) {
      await this.port.write(
        `TRACe${traceId}:MODE ${encodeTraceMode(config.mode)}`,
      );
    }
    if (config.detector) {
      await this.port.write(
        `SENSe:DETector${traceId} ${encodeDetector(config.detector)}`,
      );
    }
    if (typeof config.enabled === "boolean") {
      await this.port.write(
        `TRACe${traceId}:STATe ${config.enabled ? "ON" : "OFF"}`,
      );
    }
  }
  ...
  async getInput(): Promise<SpectrumAnalyzerInputState> {
    const [attRaw, autoRaw, preampRaw] = await Promise.all([
      ...
      this.input.preamp.available
        ? safeQuery(this.port, "SENSe:POWer:RF:GAIN:STATe?")
        : Promise.resolve("0"),
    ]);
    return {
