...
const BASE_WAVEFORMS: readonly SignalGeneratorWaveformType[] = [
  "sine",
  "square",
  "ramp",
  "pulse",
...
/**
 * GW Instek AFG-2000 / AFG-3000 / MFG-2000 signal-generator driver.
 *
 * Follows the SCPI-1999 `SOURce<n>:FUNCtion` surface shared with
 * Tektronix AFG / Keysight Trueform / Siglent SDG. GW-specific quirks:
 *   - Noise is `NOIS` (not `PRNoise`).
 *   - Arbitrary playback uses `FUNCtion USER`.
 *   - Output impedance is `OUTPut<n>:LOAD` with `INF` for high-Z.
 *
 * AFG-2005 (entry) has no arbitrary memory and no preset slots; gated
 * by profile capabilities.
 */
export class GwInstekAfg implements ISignalGenerator {
  ...
  constructor(
    ...
    const waveforms = profile.hasArbitrary
      ? [...BASE_WAVEFORMS, "arbitrary" as const]
      : BASE_WAVEFORMS;
  ...
  async getChannelState(channel: number): Promise<SignalGeneratorChannelState> {
    ...
    const [fnRaw, freqRaw, ampRaw, offsetRaw, phaseRaw, enRaw, impRaw] = await Promise.all([
      ...
      safeQuery(this.port, `${src}:AMPLitude?`),
      safeQuery(this.port, `${src}:DCOffset?`),
  ...
  async setWaveform(channel: number, config: SignalGeneratorWaveform): Promise<void> {
    ...
    if (config.type !== "dc") {
      await this.port.write(`${src}:AMPLitude ${config.amplitudeVpp}`);
    }
    await this.port.write(`${src}:DCOffset ${config.offsetV}`);
    if (config.type === "square" && config.dutyPct !== undefined) {
      await this.port.write(`${src}:SQUare:DCYCle ${config.dutyPct}`);
    }
  ...
  async setOutputImpedance(
    ...
    const value = impedance === "highZ" ? "INF" : "50";
  ...
  async savePreset(slot: number): Promise<void> {
    if (!this.presets) {
      throw new Error(`AFG ${this.profile.variant} has no preset memory`);
    }
  ...
  #validate(
    ...
    if (!limits.supportedWaveforms.includes(config.type)) {
      throw new RangeError(
        `GW Instek ${this.profile.variant} does not support waveform ${config.type}`,
      );
    }
    if (config.type !== "dc") {
